import time
from datetime import datetime, timezone
from astropy.time import Time
import tzlocal
#import pytz


# fast data import from iso format
import ciso8601 as ciso



# set local timezone and tz aware epoch!
local_tz = tzlocal.get_localzone() 
epoch = datetime.fromtimestamp(0).astimezone(timezone.utc)

# version about 10 time faster than using Time
def kk2epoch(s, tz=local_tz):
	"""
	Convert the timestamp generated by the K+K counters to epoch.
	Note that the timestamp is in local time.
	
	Parameters
	----------
	t       : timetag
	tz	: timezone, default=local system timezone	
		
	Returns
	-------	
	out     : total_seconds from the epoch
	
	Example
	-------
	
	Use it as a converter from K+K file:
	>>> import tintervals as ti
	>>> data = genfromtxt(fili, delimiter='  ', skip_header=3, skip_footer=1, converters = {0: ti.kk2epoch}, invalid_raise=False)

	
	"""


	# lets try to use ciso8601 to get faster parsing of huge comb file
	# i neet to add the first 2 digits of the year and replace the * with a space (or a T)
	s = "20" + s.replace("*", " ")
	date = ciso.parse_datetime(s)
	
	# set local timezone
	if tz:
		date = date.astimezone(tz)

	out = (date- epoch).total_seconds()
	return out



def epoch2mjd(epoch):
	return Time(epoch, format='unix').mjd

def mjd2epoch(mjd):
	return Time(mjd, format='mjd', scale='utc').unix
	
	
	
	
	
# test


