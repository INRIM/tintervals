...
def array2intervals(t, tgap=1.0, tblock=0.0):
    """
...
def regvals(tstart, tstop, base=1.0, offset=0.0, extend=True):
...
def raverage(data, t, base=1.0, offset=0.0, step=None, gap_ext=10):
...
def maverage(data, t, intervals):
    ...
    for a, b in intervals:
        ...
        if den > 0:
            ...
            res += [num * 0.0]  # garbage data, will be masked away later but I avoid a warning
...
def csaverage(f, ti, to, axis=0):
    ...
    npad = [
        (0, 0)
    ] * f.ndim  # https://stackoverflow.com/questions/19349410/how-to-pad-with-zeros-a-tensor-along-some-axis-python
    pad_size = len(tiadd)
